function [cc] = pathforpointclouds2(blob)
...
mednorsize = 17;
vx = medfilt2(vx,[mednorsize,mednorsize]);
vy = medfilt2(vy,[mednorsize,mednorsize]);
vz = medfilt2(vz,[mednorsize,mednorsize]);
...
for iplot = 1:size(cc,2)
    if isempty(cc{iplot}) ~=1  %double check if the path is empty
        ...
        hold on
%         plot normal
        npPlot = npPlot+size(cc{iplot},1);
        for inormal = 1: size(cc{iplot},1)
            plot3([cc{iplot}(inormal,1), cc{iplot}(inormal,1)+ cc{iplot}(inormal,4)*4], ...
                  [cc{iplot}(inormal,2), cc{iplot}(inormal,2)+ cc{iplot}(inormal,5)*4], ...
                  [cc{iplot}(inormal,3), cc{iplot}(inormal,3)+ cc{iplot}(inormal,6)*4],'r')
        end
